...
scrapings
3.Scrape with page numbers: given a list of urls, each with multiple pages. scrape every page using a given list of pages as the suffix for urls
...
class Crawler:
    ...
    def singleScrapeMain(self, urls, outputFileName, prefix=""):
        """
        ...
        for counter, url in enumerate(urls):
            ...
            data, status = util.search(url, self.log)
            if status == 200:
                output[url] = data
        util.save_text(outputFileName, output)

    def multiScrapeMain(self, initUrls, iterations, regexpList, outputFileName, prefixs=None, prefix_single=""):
        """
        ...
        while currIter < iterations:
            ...
            for counter, url in enumerate(urls):
                url = prefix + url
                result, status = util.search(url, self.log)
                if status == 200:
                    new_list += util.lookForPatterns(result, regexp)
                util.consoleLog(2, url, counter, len(urls), m2i=currIter, m2t=iterations)
            urls = new_list
            currIter -= 1
        self.singleScrapeMain(urls, outputFileName, prefix_single)

    def pagesScrapeMain(self, urls, pages, outputFileName, prefix=""):
        """
        ...
        for c_u, url in enumerate(urls):
            ...
            for c_p, page in enumerate(pages):
                url_page = url + page
                result, status = util.search(url_page, self.log)
                util.consoleLog(3, url_page, c_u, len(urls), c_p, len(pages))
                if status == 200:
                    output[url_page] = result
        util.save_text(outputFileName, output)

    # def yandexSearch(self, api_user, api_key, top_k_results, prefixs, suffixs, outputFileName):
    #     """
